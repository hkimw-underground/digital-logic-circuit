...
```mermaid
flowchart LR
    S1["슬라이드 1<br/>표지"] --> S2["슬라이드 2<br/>개발 배경"]
    S2 --> S3["슬라이드 3<br/>시스템 구조"]
    S3 --> S4["슬라이드 4<br/>1차 인증<br/>NFC / PIN"]
    S4 --> S5["슬라이드 5<br/>2차 인증<br/>YOLOv8 얼굴 인식"]
    S5 --> S6["슬라이드 6<br/>관리자 대시보드"]
    S6 --> S7["슬라이드 7<br/>보안 및 최적화"]
    S7 --> S8["슬라이드 8<br/>기대 효과 및 향후 계획"]
    S8 --> S9["슬라이드 9<br/>Q&A 및 시연"]
```
...
- 핵심 요소: 하드웨어(아두이노, NFC 리더, 키패드, 릴레이), 소프트웨어(FastAPI, YOLOv8, SQLite), 통신(시리얼 프로토콜)

### 슬라이드 4: 1차 인증 - NFC 및 PIN

- MFRC522 라이브러리로 NFC 카드 UID를 읽는 과정을 설명합니다.
- bcrypt(비밀번호를 복원할 수 없는 형태로 저장하는 암호화 방식) 적용으로 데이터베이스 유출 시에도 안전함을 강조합니다.
...
- 스마트폰 화면을 이용한 스푸핑 공격 차단과 눈 깜빡임 감지(Liveness 판별)를 강조합니다.
...
- 향후 계획: 시리얼 통신 암호화 적용, 모바일 앱 연동, 얼굴 인식 모델 경량화
