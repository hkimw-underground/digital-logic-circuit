...
```mermaid
flowchart TD
    A["pickle 라이브러리로<br/>얼굴 데이터 저장"] --> B["취약점 발견<br/>악성 데이터 삽입 시<br/>서버 제어권 탈취 가능"]
    B --> C["원인 분석<br/>pickle은 코드 자체를<br/>실행할 수 있어 위험"]
    C --> D["pickle 코드 전량 삭제"]
    D --> E["안전한 저장 방식 도입<br/>FACE128F64 접두사 + 숫자만 파싱"]
    E --> F["Fail-Safe 원칙 적용<br/>애매하면 문을 열지 않는다"]
```
